...
EBASE demonstrated on synthetic and obseved data that reasonable estimates .. EBASe was developed to address several critical needs in existing methods for estimating metabolism by 1) using a Bayesian framework, and 2) modifying several components of the underlying model with parameterizations more appropriate for estuarine settings. First, the Bayesian framework follows a an approach where the model is fit to the available data and allow for the incorporation of prior knowledge in teh model fitting process. ... The advantages of such an approach has led to widespread adoption of Bayesian approaches over frequentist approaches, especially in the coastal and marine community,  yet they have not been explored in do data for estuaries.  We have demonstrated that even using uninformed priors, EBASE can reproduce .  However, an exception is b which was unstable . This result may be explained simply by lack of gas exchange signal in teh DO time series that could be dominated by production and respiration.  Caffrey et al. suggest... EBASE could be applied in other settings where gas exchange may be expected to have a strong influence (windy). Or, more constrained priors could be used.  

The second improvement of EBASE over alternative methods was the several modifications to .... Primary modifications (Production is linearly related to PAR (aPAR), not saturation point Respiration is constant (not temp dependent), Uses a gas exchange commonly used in oceanic systems (not littoral - streams/lakes as in Grace)).  this also reduces the data requirements by not needing... 
