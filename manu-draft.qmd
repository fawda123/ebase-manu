...
```{r}
#| label: ebaseiotab
#| tbl-cap: "EBASE model parameters showing the required input, parameters derived from the model input, and model output.  The model notation shows the components decribed in @eq-ebase.  The model notation for production, respiration, and gas exchange shows both the simple parameter name and the components of @eq-ebase used to estimate the parameter."
ebaseiotab
```
