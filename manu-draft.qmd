...
The Estuarine BAyesian Single station Estimation (EBASE) method provides a simple approach to quantify metabolic parameters from continuous dissolved oxygen time series.  The estimates are based on a modified version of the mass balance equation described in @Grace15.  The original mass balance equation is:

$$
\frac{\delta{C_d}}{\delta{t}} = [\,aPAR^p]\, - [\,r\Theta^{T - \bar{T}}]\, - \left[-K\cdot1.0241^{T-\bar{T}}\left(C_{sat} - C_d \right )\right]
$$ {#eq-base}

where the terms in the square brackets are production (P), respiration (R), and gas exchange (D) as a function of the change in dissolved oxygen ($\delta{C_d}$) per unit time of the input data ($\delta{t}$).  Five unknown parameters are estimated by the model: $a$, $p$, $r$, $\Theta$, and $K$.  The required inputs are the time series of diel oxygen ($C_d$, mmol/m$^2$), surface irradiance ($PAR$, $\mu$mol/m$^2$/s), water temperature ($T$, C), and oxygen saturation ($C_{sat}$, mmol/m$^3$). 
Dissolved oxygen at saturation $C_{sat}$ is calculated from the input data as a function of water temperature and salinity (psu) [@Garcia92]. @eq-ts is temperature scaled, as input to @eq-co that calculates the solubility of dissolved oxygen per volume of seawater at the given temperature. The coefficients are $A_0$ = 5.80818, $A_1$ = 3.20684, $A_2$ = 4.11890, $A_3$ = 4.93845, $A_4$ = 1.01567, $A_5$ = 1.41575, $B_0$ = -7.01211e-3, $B_1$ = -7.25958e-3, $B_2$ = -7.93334e-3, $B_3$ = -5.54491e-3, and $C_0$ = -1.32412e-7.
...
where $U_{10}^2$ is the wind speed at 10-m height and $S_c$ is the Schmidt number defined as the ratio of the kinematic viscosity of water to the molecular diffusivity of the gas for oxygen calculated as a function of water temperature and salinity using the polynomial fit of @Wanninkhof14.  The parameter $b$ ($\buni$) must be estimated from the data. The other variables are as defined as in @eq-base.

The final modification to @eq-base for EBASE is a simplification to remove the $\Theta$ and $p$ parameters.  For EBASE, the temperature dependency of respiration is removed by setting $\Theta^(T-\bar{T}$ = 1) and $p$ is also set as 1.  This further simplifies @eq-basewan to:
...
For ease of application, an R package was developed to implement the EBASE methods described above.  
