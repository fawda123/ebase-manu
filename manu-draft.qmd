...
![Comparison of EBASE results for the best performing model with known metabolic results from a simulated time series created with Fwoxy. The best performing model for the one-day (left, ndays = 1) and seven-day (right, ndays = 7) optimization period is based on the EBASE results with prior distributions that produced the highest mean $R^2$ values across all parameter estimates (@fig-apasumdat).  The standard deviations of the prior distributions of each parameter for the best performing model are shown at the top.  The results are averaged at the time step of the model optimization period.  The $b$ parameter is fixed at 0.251 $\buni$.](figs/optex.png){#fig-optex fig-align="center"}
