...
![Simulated time series using the Fwoxy model for one year of continuous data at Apalachicola Bay.  Simulated input to EBASE was the (a) dissolved oxygen time series, with results compared to the (b) simulated $a$ parameter and (c) simulated metabolic estimates for production (P), respiration (R), and gas exchange (D).](figs/simapa.png){#fig-simapa}
